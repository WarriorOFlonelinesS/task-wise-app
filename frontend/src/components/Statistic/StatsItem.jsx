...
export const StatsItem = ( {label, value, icon, max=100, thresholds = { warning: 50, critical: 80}} ) => {
  ...
  return (
    <div className='flex flex-col items-center justify-center backdrop-blur-md bg-white/5 border border-white/10 rounded-full w-33 h-33 shadow-2xl transition-all hover:bg-white/10 group'>
      ...
      <div className='absolute inset-0 flex flex-col items-center justify-center text-center'>
        <span className={`text-4l font-black tabular-nums transition-colors ${statusClasses.split(' ')[0]}`}>
          {displayValue}
        </span>
        <span className="text-gray-400 text-xs font-medium uppercase tracking-tighter mt-1">
          {label}
        </span>
        <div className='mt-2 text-white/40 group-hover:text-blue-400 transition-colors'>
          {icon}
        </div>
      </div>
    </div>
